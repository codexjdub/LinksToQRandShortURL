...
Enter a URL or paste a QR code image that contains a website link. Link Sheet shortens the destination, creates a large QR code, and prints the QR code, short link, and original URL on one US Letter page. The preview matches the print sheet.
...
When you select Generate or import a QR image, the browser sends the URL to [Spoo.me](https://spoo.me/api) for shortening. If that service is unavailable, it tries [Shrtr](https://shrtr.top/api). Both services allow browser requests without an API key. Shortening needs an internet connection and is subject to their rate limits. If both fail, the QR code still opens the original URL and the page shows a warning. QR image reading, QR creation, SVG download, preview, and printing work offline.
